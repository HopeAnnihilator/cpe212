...
void Graph::DepthFirstSearch(string startVertex, string endVertex, queue<string>& path) {
    ...
    for (edges.push(startVertex); !edges.empty() && edges.top() != endVertex;) {
        ...
        // get edges, if no edges pop stack to return to previous node
        // else push queue to stack and clear queue
        GetToVertices(edges.top(), q);
        if (q.empty()) edges.pop();
        else for (; !q.empty(); q.pop()) {edges.push(q.front());}
    }
    // if no edges, set path to empty and return
    // else push a single string containing all nodes to path
    // if (!edges.empty()) {
    //     for (path.push(edges.top()), path.push(path.front()), path.pop(); \
    //     path.size() > 1; \
    //     path.pop()) 
    //     {
    //         path.back().insert( \
    //             path.back().size(),  " " + path.front() \
    //         );
    //     }
    // }
    if (!edges.empty()) path.push(edges.top());
    else for (; !path.empty(); path.pop()) {}
}